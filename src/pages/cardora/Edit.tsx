...
import { getCardoraById, updateCardoraById } from '@/firebase';
...
const Edit = () => {
  ...
  const [cardora, setCardora] = useState<(FormData & { id: string }) | null>(null);
  const [validUser, setValidUser] = useState<boolean | null>(null);
  const PLACEHOLDER_IMAGE = `https://api.dicebear.com/9.x/thumbs/svg?seed=${cardora?.firstName}+${cardora?.lastName}`;
  ...
  useEffect(() => {
    ...
    const fetchCardora = async () => {
      try {
        ...
        setCardora({ ...data, id: decryptedId } as FormData & { id: string });
      } catch (error) {
        console.error('Error fetching Cardora:', error);
      }
    };
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      await updateCardoraById(decryptedId, updatedData);
      setCardora({ ...updatedData, id: id });
    } catch (error) {
      console.error(
        'Error while updating the Cardora:',
  ...
  if (validUser === false) {
    return (
      <Layout
        title="Access Denied"
        description="You don't have permission to edit this Cardora"
        centerContent
      >
        <AlertCard
          icon={<Restricted />}
          title="Access Denied"
          message="It looks like you don't have permission to edit this Cardora."
    ...
    if (isLoading && !error) {
      return (
        <div className="flex flex-col items-center space-y-4">
          <Spinner />
          <h2 className="text-base">Updating your Cardora...</h2>
        </div>
      );
    }
    ...
    return (
      <div className="flex flex-col items-center space-y-8">
        <div className="relative overflow-hidden aspect-square rounded-xl size-52">
          ...
          <img
            src={cardora?.avatarUrl || PLACEHOLDER_IMAGE}
            alt={`${cardora?.firstName} ${cardora?.lastName}`}
            draggable="false"
            loading="eager"
            decoding="async"
        ...
        <div className="space-y-0.5 text-center">
          <h2 className="text-lg font-medium">
            Hey, {cardora?.firstName} {cardora?.lastName}!
          </h2>
          <p className="text-xs text-stone-400">Your Cardora has been successfully updated.</p>
        </div>
  ...
  return (
    <Layout title="Edit" description="Update your Cardora details!" centerContent>
      <form className="w-full max-w-screen-md" onSubmit={handleSubmit(onSubmit)}>
        <Fieldset className="space-y-6">
          <div className="flex justify-between w-full">
            <FormLegend title="Edit Your Cardora" description="Modify the fields as needed." />
            <AvatarField
              id="avatar"
              name="avatar"
              register={register}
              error={errors.avatar as FieldError}
              hasUrl={cardora?.avatarUrl}
            />
          </div>
