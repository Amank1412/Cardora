...
const Detail = () => {
  ...
  const [cardora, setCardora] = useState<FormData | null>(null);
  ...
  useEffect(() => {
    const fetchCardora = async (decryptedId: string) => {
    ...
    if (decryptedId) fetchCardora(decryptedId);
  }, [id]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await deleteCardoraById(decryptedId);
      navigate('/collection', { state: { deleted: true } });
    } catch (error) {
      console.error(
        'Error deleting Cardora:',
        error instanceof Error ? error.message : 'Unknown error'
      );
    }
  };
  ...
  if (!cardora) {
    return (
      <Layout title="Cardora Not Found!" centerContent withHeader={false}>
        <AlertCard
          icon={<Question />}
          title="Oops! This Cardora is Missing"
          message="We couldn't locate this Cardora. It may have been deleted or never existed."
