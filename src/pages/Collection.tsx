...
import { getStoredCardoras } from '@/firebase';
...
const Collection = () => {
  ...
  useEffect(() => {
    const fetchFromDB = async (storeName: string) => {
      ...
      return data?.length ? await getStoredCardoras(data) : [];
    };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Error fetching Cardoras:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Layout
        ...
        description="Please wait while we fetch your saved Cardoras"
        centerContent
      >
        <Spinner />
      </Layout>
    ...
    return (
      ...
      >
        <AlertCard
          ...
          message="You haven't created any Cardoras. Kickstart your collection and make connections with ease!"
        >
  ...
  return (
    <Layout title="Collection" className="pt-32 pb-8">
      {deleted && (
        <Toast
          message="Your Cardora has been successfully deleted."
